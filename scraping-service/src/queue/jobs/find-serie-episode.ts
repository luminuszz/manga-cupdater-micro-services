...
export const findSerieEpisodeJobToken = 'find-serie-episode';
...
@Processor(findSerieEpisodeJobToken)
export class FindSerieEpisodeJob {
  private logger = new Logger(FindSerieEpisodeJob.name);
  ...
  public async execute({
    ...
    try {
      this.logger.log(`Initi job for ${name} - ${id}`);
  ...
  async onJobCompleted({ returnvalue }: Job) {
    const { hasNewEpisode, name, newEpisode, id, url } =
    ...
    if (hasNewEpisode) {
      ...
      await this.messagingService.notifyNewEpisode({
    ...
    this.logger.log(`No new episode for ${name} - ${id}`);
  }
}
